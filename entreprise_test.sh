...
# 5. TEST SÉCURITÉ : ACCÈS SSH BLOQUÉ (Exemple de règle DROP)
draw_box "SÉCURITÉ : FILTRAGE DES PORTS (SSH)"
echo -e "${NC}Tentative de connexion SSH vers Serveur_Entreprise_1 (devrait être bloqué)..."
# nc -z (zero-I/O mode) avec un timeout de 1 seconde
CMD="docker exec Client_Ent1 nc -z -w 1 10.10.20.2 22"
echo -e "${NC}Exécution : ${YELLOW}$CMD${NC}"

if eval $CMD > /dev/null 2>&1; then
    echo -e "${RED}❌ ALERTE : Le port SSH est OUVERT (Faille de sécurité !)${NC}"
else
    echo -e "${GREEN}✅ SUCCÈS : Le port SSH est FERMÉ par le Firewall.${NC}"
fi
echo ""

# 6. TEST SÉCURITÉ : ISOLATION DMZ -> LAN
draw_box "SÉCURITÉ : ISOLATION DMZ VERS LAN"
echo -e "${NC}Le serveur Web tente d'accéder au serveur RH (devrait être bloqué)..."
CMD="docker exec Serveur_Entreprise_1 ping -c 1 -W 1 10.10.10.5"
echo -e "${NC}Exécution : ${YELLOW}$CMD${NC}"

if eval $CMD > /dev/null; then
    echo -e "${RED}❌ ALERTE : La DMZ peut communiquer avec le LAN RH !${NC}"
else
    echo -e "${GREEN}✅ SUCCÈS : La DMZ est bien isolée du LAN.${NC}"
fi
