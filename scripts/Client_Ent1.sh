...
docker exec --privileged Client_Ent1 ip route add default via 10.10.10.254

# Définir le serveur DNS interne
docker exec --privileged Client_Ent1 sh -c "echo 'nameserver 10.10.20.2' > /etc/resolv.conf"
